...
class SecurityServiceProvider extends CProvider
{
	...
	public function register(Container $app)
	{
		...
		$app['security.user_provider.admin'] = function() use ($app) {
			return new PlatformUserProvider($app->getServiceUsers(), $app->getServiceUsersAuth());
		};

		$app['security.user_provider.public'] = function() use ($app) {
			return $app['security.user_provider.admin'];
		};

		$app['security.encoder_factory'] = function() use ($app) {
		...
		$app['security.authentication.failure_handler.admin'] = function() use ($app) {
		...
		$app['security.authentication_listener.logout._proto'] = $app->protect(function($name, $options) use ($app, $logoutProto) {
			return function() use ($app, $name, $options, $logoutProto) {
		...
		// Security voters.
		$app['security.voters.entity_erase'] = function($app) {
